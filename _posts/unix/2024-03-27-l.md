...
간단한 선형 회귀 모델을 구현한 것이다. x_train, y_train에 데이터를 정의하고 가중치와 편향을 초기화한다. 비용 함수는 손실을 계산한다. SGD(확률적 경사 하강) 옵티마이저를 생성하고 for 루프를 이용해 학습 스텝을 진행한다. 매 200번째 스텝마다 현재 스텝 번호, 비용, 가중치, 편향을 출력한다. 주어진 입력 출력에 대해 가장 적합한 선형 모델의 가충치와 편향을 찾는 과정을 보여준다.
